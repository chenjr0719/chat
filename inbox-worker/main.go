...
func main() {
	...
	cctx, err := cons.Consume(func(m oteljetstream.Msg) {
		// jobguard recovers handler panics — the Consume callback runs outside
		// natsrouter's Recovery middleware, so an unrecovered panic would crash
		// the worker and crash-loop on JetStream redelivery.
