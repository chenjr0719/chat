...
//
// site selects the JetStream domain via jetstream.NewWithDomain so a
// single admin NATS connection can reach both site-a and site-b.
func (p *JetStreamConsumePoller) PollFn(site string, args map[string]any, tp string) func() []readers.Event {
	...
	inner, err := p.getOrOpen(site, stream, filter)
	if err != nil {
...
// or opens a fresh one. Holds the poller's mutex for the duration of
// the open call so two concurrent assertions on the same args don't
// race to create two consumers.
func (p *JetStreamConsumePoller) getOrOpen(site, stream, filter string) (*StreamPoller, error) {
	...
	rdr := readers.NewJetStreamSubjectReaderWithDomain(p.conn, site, stream, filter, "jetstream_consume", "")
